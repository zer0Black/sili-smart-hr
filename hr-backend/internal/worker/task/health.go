...
// NewMux 构造 worker 任务路由：健康任务原地注册，抽取任务经参数注入（保持单一注册入口）。
// session-extract 以 context.WithTimeout 挂任务级超时（asynq v0.26.0 ServeMux 无
// options 注册 API），预算声明见 session_extract.go 的 sessionExtractTimeout。
func NewMux(sessionExtract asynq.HandlerFunc) *asynq.ServeMux {
