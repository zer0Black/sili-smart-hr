...
// personEvaluateTimeout 任务级超时 1050s（03 §3.3 推导）：LLM 段最坏 840s（评估
// 专用 client Timeout 180s，429 带 Retry-After 封顶 60s 下单次 callOnce 420s，
// schema 校验失败重试共 2 次完整调用）+ 列表翻页与落库共享剩余 210s（每页最坏
// 190s 多页慢路径先吃预算，listMaxPages 页数上限兜底终止）。
