...
按 specs §4.1.4 规则1（选「全员」时展开为发起时点的全员名单快照）与 §5.1.2 步骤2（触发时读取评估对象快照），全员模式下的人员集合在**批次创建时**确定：创建入口经人员检索接口 `GET /api/staffs` 全量分页拉取全员名单，落 `target_names_json` 与 `total_count`，并同步按名单批量写入批次人员明细（`status=pending`）。
...
全员名单拉取失败（上游人员接口不可达）时：定时批次记 ERROR 并上抛 Asynq 任务级重试（§4.3），手动批次返回 1305 且不落批次记录（B1 错误码表）。创建入口的时序为先取名单快照、再落批次记录，故批次一经落库即持有完整名单，`total_count` 无 0 值窗口。
...
| page_size | integer | 否 | 10 | 每页数量 [可选值：10/20/50]（specs §8.3 偏离记录第 1 条） |
