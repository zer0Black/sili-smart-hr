...
- `session_features` 的字段扩展（本功能只读消费，结构变更归各自 Feature）。例外：在 `first_turn_at` 既有列上叠加的 `idx_first_turn` 纯索引（本功能授权的辅助改动），当前分子口径改为按本批次投递会话键统计后暂无生产查询消费，保留为窗口级统计的预留索引。
...
**v1.7 变更（代码评审反向同步）：** §9 `idx_first_turn` 披露改写：分子口径切换为按本批次投递会话键统计（走 `uk_session_key`）后该索引暂无生产查询消费，如实标注为窗口级统计的预留索引。
