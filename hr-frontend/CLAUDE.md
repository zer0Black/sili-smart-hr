...
本文件给在前端目录（hr-frontend/）下工作的 Claude Code 提供操作规约。项目整体定位、跨前后端约定、本地启动见上一层 [../CLAUDE.md](../CLAUDE.md)，本文件只承载前端独有约定，不重复后端内容。当前仓库处于 B 档可运行脚手架阶段，前端已贯通八个业务域：account（登录 + 用户管理）、system（初始化向导 + 系统状态）、dimension（维度与权重）、llm-config（大模型配置）、integration-secret（集成密钥，卡片内嵌大模型配置页）、system-params（评估周期配置）、assessment（评测运营中心：态势统计卡、跑批计划卡、批次列表与轮询、发起评测与失败明细弹窗）、question-bank（题库台账与批次审核：AI 题与量表题双 tab 列表，详情查看、编辑、启停、删除、驳回题重新提交；待审批次卡区与批量审核视图在列表态页内切换，审核视图支持卡片流标记驳回、确认入库与批次作废；量表引入两步弹窗：候选卡单选（已引入置灰）、只读汇总与批次号预览，成功后可携 batchId 直切审核视图；AI 生成独立页 /question-bank/generate，表单态维度多选卡加题数输入、进行态进度条显当前维度、完成态显批次号并可再生成（保留选择）、失败态按错误枚举出文案，2s 函数式轮询进度且页面不可见自动暂停），对应路由 /login、/setup 与 _authenticated 下的 /system/users、/system/status、/system/dimension、/system/llm、/system/params、/assessment、/question-bank 与 /question-bank/generate，其中 account 链路是最早的端到端样板。dashboard（仪表盘）、profile（画像）、运营日志仍是导航里的待激活占位，/answer/$token 作答页为 B 档占位页。规格权威源是 [../context/03_architecture/architecture.md](../context/03_architecture/architecture.md)，第 4 章承载运行时约定。
...
[routes/](src/routes/) 是 TanStack 文件路由的编排层，只做路由与页面入口，页面实现下沉到 feature。[features/](src/features/) 按业务域自包含，每个域三件套：`api.ts`（纯函数请求）、`hooks.ts`（TanStack Query 封装）、`types.ts`（域私有类型），现有 account、system、dimension、llm-config、integration-secret、system-params、assessment、question-bank 八个域，dimension 另有 `validation.ts`、system-params 另有 `period.ts`（周期联动纯函数）、assessment 另有 `period-default.ts`（时段默认回填纯函数）与 `target-summary.ts`（评估对象摘要纯函数），question-bank 的台账与审核组件在 `features/question-bank/components/`（批次与量表接口及 hooks 另有 `batch-api.ts`/`batch-hooks.ts`/`scale-api.ts`/`scale-hooks.ts`，AI 生成另有 `generate-api.ts`/`generate-hooks.ts`，轮询 hook 用函数式 refetchInterval 2s 加页面可见性守卫），页面专用组件在 `features/<域>/components/`。[lib/](src/lib/) 放框架级基础设施（http-client、query-client、contracts、jwt、crypto、validation、utils），跨域共享。[stores/](src/stores/) 是 Zustand store，目前只有 [auth.ts](src/stores/auth.ts)。[components/ui/](src/components/ui/) 只放 shadcn new-york 原子件（button/card/input/label/select/switch/slider/table/textarea/dialog/alert-dialog/badge/sonner），[components/](src/components/) 放业务通用件（error-boundary、nav-menu、theme-provider、auth-split-layout）。[i18n/](src/i18n/) 是国际化配置与语言资源，[styles/](src/styles/) 只有 [globals.css](src/styles/globals.css) 一个文件。
...
命名空间有 13 个：common（默认）、auth、account、errorBoundary、notFound、setup、system、dimension、llmConfig、integrationSecret、systemParams、assessment、questionBank。页面级用 `useTranslation('域')` 指定 ns，跨 ns 引用传第二参数 `{ ns: 'common' }`。[zh.json](src/i18n/locales/zh.json) 与 [en.json](src/i18n/locales/en.json) 的 key 树必须逐一对齐同步维护，漏翻会让缺失 key 回退到 zh。
