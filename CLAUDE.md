...
当前处于 B 档可运行脚手架阶段，account 登录链路（POST /api/login、GET /api/me）是首个样板，用户管理、维度配置、系统参数、大模型配置、集成密钥、题库管理等链路已前后端贯通。engine 在后端的 extractor（会话特征抽取）、activity（使用活跃度统计）、evaluator（跨会话综合评估）、scorer（多维聚合）、fallback（失败重试与告警兜底）、pipeline（跑批编排）六个子域已实现，questionbank（题库管理）域的台账维护、批次审核与量表引入链路（题目列表/详情/编辑/启停/删除，待审批次卡、批量审核确认入库、批次作废、驳回题重新提交，内置九型量表候选列表与单事务引入建批）已前后端贯通，批次生产来源已通量表引入一路、AI 生成归后续子计划，answer / profile / dashboard / workspace 等业务域前后端均未开工。新增业务域时，account 全链路（后端 domain → repository → service → handler → router → wire，前端 feature → route）是参考样板。
