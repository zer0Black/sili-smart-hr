...
本文件给在前端目录（hr-frontend/）下工作的 Claude Code 提供操作规约。项目整体定位、跨前后端约定、本地启动见上一层 [../CLAUDE.md](../CLAUDE.md)，本文件只承载前端独有约定，不重复后端内容。当前仓库处于 B 档可运行脚手架阶段，前端已贯通七个业务域：account（登录 + 用户管理）、system（初始化向导 + 系统状态）、dimension（维度与权重）、llm-config（大模型配置）、integration-secret（集成密钥，卡片内嵌大模型配置页）、system-params（评估周期配置）、assessment（评测运营中心：态势统计卡、跑批计划卡、批次列表与轮询、发起评测与失败明细弹窗），对应路由 /login、/setup 与 _authenticated 下的 /system/users、/system/status、/system/dimension、/system/llm、/system/params、/assessment，其中 account 链路是最早的端到端样板。dashboard（仪表盘）、profile（画像）、运营日志仍是导航里的待激活占位，/answer/$token 作答页为 B 档占位页。规格权威源是 [../context/03_architecture/architecture.md](../context/03_architecture/architecture.md)，第 4 章承载运行时约定。
...
命名空间有 12 个：common（默认）、auth、account、errorBoundary、notFound、setup、system、dimension、llmConfig、integrationSecret、systemParams、assessment。页面级用 `useTranslation('域')` 指定 ns，跨 ns 引用传第二参数 `{ ns: 'common' }`。[zh.json](src/i18n/locales/zh.json) 与 [en.json](src/i18n/locales/en.json) 的 key 树必须逐一对齐同步维护，漏翻会让缺失 key 回退到 zh。
