...
// sessions 可空：空时内部 StatPersonByKey 拉取，拉到的窗口内列表注入 Evaluate
//（签名识别列表口径）；非空时直传 StatPerson 免二次拉取。LLM 失败降级路径
//（Evaluate 内已落 failed 行、err=nil）继续聚合；error 通道只透传基础设施错误
//（配置/落库/列表拉取），业务态不占。
func (e *Evaluator) EvaluatePerson(ctx context.Context, tokenName string, period activity.Period, sessions []conversationlog.SessionSummary) (*EvaluateResult, error) {
	...
	var err error
	if len(sessions) == 0 {
		stat, sessions, err = e.act.statPersonByKey(ctx, tokenName, period)
	} else {
	...
	res, err := e.Evaluate(ctx, tokenName, period, sessions)
	if err != nil {
		return nil, err
	}
	res.Activity = stat
...
// statPersonByKey 适配透传：消费 StatPersonByKeyWithSessions 的列表透出形态。
func (a *activityStatAdapter) statPersonByKey(ctx context.Context, tokenName string, period activity.Period) (*activity.ActivityStat, []conversationlog.SessionSummary, error) {
