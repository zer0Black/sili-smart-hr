...
[model/migrate.go](internal/model/migrate.go) 的 `migrateDB` 三段式：A 段类型迁移钩子必须在 AutoMigrate 之前（先 Migrator 探测再按方言 ALTER，否则旧类型仍在），目前承载 session_features.client 列的存量库加列钩子（NOT NULL 无 default 的加列在有数据行的库上会被 SQLite/PG 拒绝，钩子以 `DEFAULT ''` 兜底）；B 段 AutoMigrate 建表加列加索引为主，GORM 不删列不改列类型；C 段数据回填钩子用幂等 UPDATE 放 AutoMigrate 之后，目前承载 inject_prefixes 替换语义存量行（出厂全集 JSON）一次性重置为空数组（精确匹配全集值，运维改过的行不触碰）。无版本号、每次启动重跑、靠幂等保证安全。`migrateColumnType` 是跨方言改列模板，SQLite 因类型亲和直接跳过，PG/MySQL 先查 information_schema 判断当前类型再 ALTER。SQLite 不设连接池，只有 MySQL/PG 调 applyPool（SQLite 设连接池会破坏单写特性）。
